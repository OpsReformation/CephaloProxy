...
**Chosen Approach**: Single target with platform overrides using base target
inheritance
...
## Decision: Hybrid Cache Model

**Chosen Approach**: GitHub Actions cache for CI/CD + Local cache for
development

**Rationale**:

- CI/CD: GitHub Actions cache (gha backend) - native integration, no registry
  required, persists across workflow runs
- Local development: Local filesystem cache (local backend) - full control,
  fast, no external dependencies
- Registry cache backend - Not required, avoids registry access constraints
...
**Alternative Considered**: Registry-based cache

- Requires public/private registry access
- Additional authentication setup
- Not suitable for all environments
- Rejected because it creates dependency on registry access
...
**Chosen Approach**: Fail immediately with clear error message if BuildKit not
available
