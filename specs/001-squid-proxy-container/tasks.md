...
**Tests**: Per constitution requirement (Test-First Development), tests MUST be written and approved BEFORE implementation for each user story.
...
**Goal**: Deploy functional proxy with default configuration, no volumes required

**Independent Test**: Start container without volumes, proxy HTTP traffic, verify health checks respond
...
**Independent Test**: Mount ACL config blocking domains, verify blocked traffic denied, allowed traffic passes

### Tests for User Story 2 (REQUIRED - Test-First Development)

- [ ] T030 [P] [US2] Write integration test in tests/integration/test_acl_filtering.sh for blocked domain denial
- [ ] T031 [P] [US2] Write integration test in tests/integration/test_acl_filtering.sh for allowed domain success
- [ ] T032 [P] [US2] Create test fixture in tests/fixtures/test-configs/blocked-domains.acl with sample blocked domains
- [ ] T033 [P] [US2] Create test fixture in tests/fixtures/test-configs/filtering-squid.conf with ACL configuration
- [ ] T034 [US2] Run all US2 tests and verify they FAIL (red phase)

### Implementation for User Story 2

- [ ] T035 [P] [US2] Create example ACL filtering config in config-examples/filtering/squid.conf
- [ ] T036 [P] [US2] Create example blocked domains ACL in config-examples/filtering/blocked-domains.acl
- [ ] T037 [US2] Update entrypoint.sh to detect and load configs from /etc/squid/conf.d/*.acl
- [ ] T038 [US2] Update entrypoint.sh to merge ACL includes into main squid.conf if using default config
- [ ] T039 [US2] Verify ACL denial logging in squid.conf (access_log shows TCP_DENIED)
- [ ] T040 [US2] Add volume mount documentation for /etc/squid/conf.d in quickstart.md
...
**Independent Test**: Mount SSL certs, verify HTTPS decrypted and cached, second request cache hit

### Tests for User Story 3 (REQUIRED - Test-First Development)

- [ ] T044 [P] [US3] Write integration test in tests/integration/test_ssl_bump.sh for HTTPS interception
- [ ] T045 [P] [US3] Write integration test in tests/integration/test_ssl_bump.sh for cache hit verification
- [ ] T046 [P] [US3] Generate test CA certificate in tests/fixtures/test-certs/ca.pem and ca.key
- [ ] T047 [P] [US3] Create test SSL-bump config in tests/fixtures/test-configs/sslbump-squid.conf
- [ ] T048 [US3] Run all US3 tests and verify they FAIL (red phase)

### Implementation for User Story 3

- [ ] T049 [P] [US3] Create example SSL-bump config in config-examples/ssl-bump/squid.conf with ssl_crtd settings
- [ ] T050 [US3] Update Dockerfile to compile Squid with ssl_crtd helper (--enable-ssl-crtd flag)
- [ ] T051 [US3] Update init-squid.sh to initialize SSL database (ssl_crtd -c -s /var/lib/squid/ssl_db)
- [ ] T052 [US3] Add certificate validation in entrypoint.sh (check /etc/squid/ssl_cert/ca.pem exists if ssl-bump enabled)
- [ ] T053 [US3] Add certificate permission check in entrypoint.sh (ca.key must be readable only by Squid UID)
- [ ] T054 [US3] Configure HTTPS cache_dir in example config (larger size for HTTPS content)
- [ ] T055 [US3] Add SSL-bump documentation in docs/configuration.md with certificate generation instructions
...
**Independent Test**: Mount custom squid.conf with authentication, verify auth required

### Tests for User Story 4 (REQUIRED - Test-First Development)

- [ ] T060 [P] [US4] Write integration test in tests/integration/test_custom_config.sh for custom config loading
- [ ] T061 [P] [US4] Write integration test in tests/integration/test_custom_config.sh for invalid config rejection
- [ ] T062 [P] [US4] Create test fixture in tests/fixtures/test-configs/custom-advanced.conf with auth settings
- [ ] T063 [P] [US4] Create test fixture in tests/fixtures/test-configs/invalid-syntax.conf with syntax errors
- [ ] T064 [US4] Run all US4 tests and verify they FAIL (red phase)

### Implementation for User Story 4

- [ ] T065 [P] [US4] Create example advanced config in config-examples/advanced/squid.conf with auth, cache hierarchy
- [ ] T066 [US4] Update entrypoint.sh to prefer /etc/squid/squid.conf over default if mounted
- [ ] T067 [US4] Enhance config validation in entrypoint.sh to show clear error messages from squid -k parse
- [ ] T068 [US4] Add error handling in entrypoint.sh for invalid configs (exit 1 with diagnostic output)
- [ ] T069 [US4] Test custom config scenarios: auth, custom cache sizes, parent proxy hierarchies
- [ ] T070 [US4] Add advanced configuration documentation in docs/configuration.md
...
- [ ] T075 Container-First: Verify Dockerfile builds reproducibly, health checks functional, graceful shutdown works
- [ ] T076 Security: Verify container runs as UID 1000, secrets injectable via volumes, no hardcoded credentials
- [ ] T077 Security: Verify OpenShift arbitrary UID support (test with random UID assignment, GID 0)
- [ ] T078 Observability: Verify Squid access.log and cache.log output correctly, health endpoints respond < 1s
- [ ] T079 Performance: Build container and verify startup time < 10 seconds with default config
- [ ] T080 Performance: Run basic load test to verify 1000 req/s capability (use tools/load-test.sh if created)
- [ ] T081 Squid Integration: Verify Squid version pinned in Dockerfile (check Portage package.accept_keywords)
- [ ] T082 Squid Integration: Verify squid -k parse validation runs on every container start
...
- [ ] T083 [P] Create deployment guide in docs/deployment.md (Docker, Kubernetes, OpenShift examples)
- [ ] T084 [P] Create configuration reference in docs/configuration.md (all squid.conf directives used)
- [ ] T085 [P] Create troubleshooting guide in docs/troubleshooting.md (common errors, solutions)
- [ ] T086 [P] Update README.md with badges, links to docs, quick start, constitutional compliance
- [ ] T087 [P] Create GitHub workflow in .github/workflows/build-and-test.yml (build image, run all tests)
- [ ] T088 [P] Add vulnerability scanning to CI workflow (trivy scan of container image)
...
- [ ] T091 Final test: Run complete test suite (all US1-US4 tests + health + constitutional compliance)
...
- **Constitutional Compliance (Phase 7)**: Depends on all user stories being complete
- **Documentation (Phase 8)**: Can start after US1 complete, finish after all stories done
...
**Note**: While US2-US4 build conceptually on US1 (basic proxy), they are architected to be independently testable. Each can be deployed and validated without the others being complete.
...
- All Foundational tasks marked [P] can run in parallel after Dockerfile created (T006-T009)
- Once Foundational phase completes, all user story TEST tasks can start in parallel (T013-T016, T030-T033, T044-T047, T060-T063)
...
5. Add User Story 4 → Test independently → Deploy/Demo (now with full customization)
...
- TDD is NON-NEGOTIABLE per Constitution - tests written first, fail, then implement, pass
...
- **Phase 3 (US1 - Basic Proxy)**: 17 tasks (5 test tasks + 8 implementation + 4 validation)
- **Phase 4 (US2 - Traffic Filtering)**: 14 tasks (5 test tasks + 6 implementation + 3 validation)
- **Phase 5 (US3 - SSL-Bump)**: 16 tasks (5 test tasks + 7 implementation + 4 validation)
- **Phase 6 (US4 - Custom Config)**: 15 tasks (5 test tasks + 6 implementation + 4 validation)
- **Phase 7 (Constitutional Compliance)**: 8 tasks
- **Phase 8 (Documentation)**: 9 tasks

**MVP Scope**: 29 tasks (Setup + Foundational + US1) delivers working proxy container
**Parallel Opportunities**: 25+ tasks marked [P] can be parallelized
**Independent Testing**: Each of 4 user stories has clear independent test criteria
